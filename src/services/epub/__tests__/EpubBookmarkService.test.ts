...
describe("EpubBookmarkService", () => {
	...
	it("migrates legacy bookmark files onto the canonical stableKey path", async () => {
		...
		const app = {
			vault: {
				...
				getFiles: vi.fn(() => [
					{
						path: "Weave EPUB Reader/Demo--epubsrc-legacy.md",
		...
		vi.spyOn(service as any, "readBookmarkFileByPath").mockImplementation(
			...
			if (filePath !== "Weave EPUB Reader/Demo--epubsrc-legacy.md") {
				return null;
			}
			return {
		...
		const result = await (service as any).migrateBookmarkFileForBook(
			...
			"Weave EPUB Reader/Demo--epubsrc-legacy.md"
		);
		const expectedStableKey = "epubsrc-4a9ad58db18a";
		const expectedPath = "Weave EPUB Reader/data_Demo.md";
	...
	it("ignores ENOENT when removing a stale bookmark file during migration", async () => {
		...
		const app = {
			vault: {
				adapter: {
					exists: vi.fn(async (path: string) => path === "Weave EPUB Reader/Demo--legacy.md"),
					remove: vi.fn(async () => {
		...
		await expect(
			(service as any).migrateBookmarkFileForBook(
				...
				"Weave EPUB Reader/Demo--legacy.md"
			)
		).resolves.toBe("Weave EPUB Reader/data_Demo.md");

		expect(writeBookmarkFile).toHaveBeenCalled();
		expect(app.vault.adapter.remove).toHaveBeenCalledWith("Weave EPUB Reader/Demo--legacy.md");
	});
	...
	it("falls back to adapter.write when vault.modify hits ENOENT for a stale TFile index", async () => {
		...
		const app = {
			vault: {
				...
				getAbstractFileByPath: vi.fn(() => ({ path: "Weave EPUB Reader/Demo--fp-demo-123.md" })),
				modify: vi.fn(async () => {
					const error = new Error("ENOENT: no such file or directory") as Error & { code?: string };
					error.code = "ENOENT";
					throw error;
		...
		const service = new EpubBookmarkService(app);
		await (service as any).writeBookmarkFile("Weave EPUB Reader/Demo--fp-demo-123.md", {
			format: "weave-epub-bookmarks/v1",
			weave_epub_bookmark_file: true,
		...
		expect(adapterWrite).toHaveBeenCalledWith(
			"Weave EPUB Reader/Demo--fp-demo-123.md",
	...
	it("keeps bookmark filenames stable and resolves existing files by stableKey suffix", async () => {
		...
		const service = new EpubBookmarkService({
			vault: {
				...
				getFiles: vi.fn(() => [
					{
						path: "Weave EPUB Reader/史蒂夫•乔布斯传(Steve Jobs_A Biography)--fp-demo-123.md",
		...
		).resolves.toBe("Weave EPUB Reader/史蒂夫•乔布斯传(Steve Jobs_A Biography)--fp-demo-123.md");
	});
});
